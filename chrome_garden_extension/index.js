var a = [];
console.log(chrome.runtime.getURL('/svg/0.svg'));
(async () => {
	
	//get dimensions
	const scale = (await chrome.storage.sync.get(["scale"])).scale || 20;
	...
	// Source - https://stackoverflow.com/a/60251755 ----------+
	// Posted by shivampip                                     |
	...
	for (let i = 0; i < images.length; i ++) {
		let q = new Image();
		q.src = chrome.runtime.getURL(images[i]);
		await loadImage(q);
		cache.push(q);
		a.push(cache[i]);
	}
	...
	var I=setInterval(() => {gen(false);act()}, (await chrome.storage.sync.get(["sonic"])).sonic ? 16 : 128);
	document.body.style.backgroundColor = (await chrome.storage.sync.get(["bkc"])).bkc || "white";
})();
